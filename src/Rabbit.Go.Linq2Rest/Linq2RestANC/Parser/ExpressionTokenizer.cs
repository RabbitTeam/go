...
namespace Linq2Rest.Parser
{
	...
	internal static class ExpressionTokenizer
	{
	    ...
	    private static readonly Regex InFunctionRx = new Regex(@"^(in)\((.+),(.*)\)$", RegexOptions.Compiled);
	    ...
	    public static TokenSet GetInFunctionTokens(this string filter)
	    {
	        CustomContract.Requires(filter != null);

	        var functionMatch = InFunctionRx.Match(filter);
	        if (!functionMatch.Success)
	        {
	            return null;
	        }

	        var method = functionMatch.Groups[1].Value;
	        var propertyName = functionMatch.Groups[2].Value;
	        var valueString = functionMatch.Groups[3].Value;
